...
exports.miniBuffer = function(isPassword) {
    ...
    this.spliceMessage = function(start, deleteCount, items) {
        if(items) {
            var args = items.splice();
            _message.splice.apply(_message, [start, deleteCount].concat(items))
        } else {
            _message.splice(start, deleteCount)
        }
    };
